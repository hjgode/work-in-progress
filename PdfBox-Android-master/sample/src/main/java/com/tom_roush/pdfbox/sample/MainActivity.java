...
//		}
	...
	public void renderFile(View v) {
		...
		try {
			...
			PDDocument document = PDDocument.load(assetManager.open("edl_test_bt_printer.pdf"));
    ...
    public void renderFileThread(View v) {
	    new Thread(new Runnable() {
            @Override
            public void run() {
                Looper.prepare();
                // Render the page and save it to an image file
                final String path = root.getAbsolutePath() + "/Download/render.jpg";
                try {
                    // Load in an already created PDF
                    PDDocument document = PDDocument.load(assetManager.open("edl_test_bt_printer.pdf"));
                    // Create a renderer for the document
                    PDFRenderer renderer = new PDFRenderer(document);
                    // Render the image to an RGB Bitmap
                    pageImage = renderer.renderImage(0, 4, Bitmap.Config.RGB_565);
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            tv.setText("Rendering failed for "+path+ ", "+e1.getMessage());
                        }
                    });
                }
